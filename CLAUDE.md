...
- 正文：12-14pt `#333333`
- 最小字号：尽量 >= 10pt（软约束，内容完整显示优先，允许更小）
- 英文 Arial | 中文 Microsoft YaHei | 代码 Courier New

### ⚠️ 视觉校验（最高优先级）
**完整可见 > 合理布局 > 字体大小**
- 所有文本必须完整可见，不允许截断或溢出（**硬约束**）
- 文本尽量不换行；换行后最后一行 < 20% 内容 = widow line，必须修复（**硬约束**）
- 字体大小是**软约束**，为满足完整可见可缩至任意合理大小，字体永远让步于内容可见性
- 每个 .pptx 交付前必须通过 `~/.claude/plugins/marketplaces/csa-skills/scripts/validate_visual.py` 验证
